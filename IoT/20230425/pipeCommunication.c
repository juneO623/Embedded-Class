...
int main(int argc, char **argv)
{
	pid_t pid;
	int pfd[2];
	char line[BUFSIZ];		// stdio.h 파일에 정의된 버퍼 크기로 설젖ㅇ
	int status;
	...
	if ((pid = fork()) < 0) {		// fork() 함수를 이용해서 프로세스 생성
		...
		close(pfd[0]);				// 읽기를 위한 파일 디스크립터 닫기
		waitpid(pid, &status, 0);	// 자식 프로세스의 종료를 기다리지
	}
